    ...
    >
      ...
      <CardContent sx={{ margin: 1, display: { xs: "block", lg: "flex" } }}>
        <div>
          ...
          {job.acceptedBid ? (
            <p>
              <b>Pricet:</b> {job.acceptedBid?.price}$
            </p>
          ) : (
            <p>
                ...
                defaultValue={
                  Object.keys(Status)[
                    Object.values(Status).indexOf(
                      job.acceptedBid?.status as Status
                    )
                  ]
                }
              ></TextField>
              <p>
                <b>Propose description:</b> {job.acceptedBid?.description}
              </p>
            </FormControl>
            <button
              className="secondary-button"
              style={{
                color: job.acceptedBid?.status == "Ready" ? "#64aeb9" : "grey",
              }}
            >
              Confirm
            </button>
